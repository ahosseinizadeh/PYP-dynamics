...
nN = 1000;        % # of nearest-neighbors in used for embedding
...
titleSingulars = sprintf(['\\fontsize{16}{\\color{red}',myTitle,', singular values}\n'...
  '\\fontsize{14}{(nS=',int2str(nS),', nC=',int2str(nC),', nN=',int2str(nN),')}']);              
              
titleTopos =sprintf(['\\fontsize{16}{\\color{red}',myTitle,', Topos}\n' ...
  '\\fontsize{14}{(nS=',int2str(nS),', nC=',int2str(nC),', nN=',int2str(nN),')}']);
...
for cr=1:nChrono
   ...
   %plot(Chrono,'LineWidth',3,'Color',color); hold on;

   set(gca,'linewidth',2);
 %if cr==1
 % ylim([-2 0]); 
 %else
 % ylim([min(Chrono)-abs(min(Chrono))/10,max(Chrono)+abs(max(Chrono))/10]);
 %end
 %xlim([0 size(V_dataSpace,1)+200]);
 %xlim([-20,475]);
   yLabel = ylabel(['V',int2str(cr)],'FontSize',20,'FontWeight','bold','Color','k');
   set(yLabel,'Rotation',0, 'Units', 'Normalized', 'Position', [-0.08, 0.5, 0]);
%   if cr==1, ylim([0.93 1.07]); end
   if mod(cr,ell+1)~=0
     %set(gca,'xTick',[])
     set(gca,'Xticklabel',[])
   end
